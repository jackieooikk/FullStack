...
app.get("/posts/:id", (req, res) => {
  const searchIndex = posts.findIndex((post) => post.id === parseInt(req.params.id));
  if (searchIndex >= 0) {
    res.json(posts[searchIndex]);
  }  else {
    res.status(404).json({error: `Could not find post with id ${req.params.id}`});
  }
})
...
app.patch("/posts/:id", (req, res) => {
  ...
  const searchIndex = posts.findIndex(post => post.id === idToPatch);
